...
static bool writeDataToBuffer(Buffer *buffer, StatusContainer *cont) {
    ...
    if (out && cont->act) {
        ...
        out = writeDataWithSize(buffer,(TPointer*)(&cont->state),sizeof(TState   )) && out;
...
StatusContainer* readStatusContainerFromBuffer(Buffer *buffer) {
    ...
    if (out) {
        ...
        out->state = (EState   )readDataInt(buffer,sizeof(TState   ));
